...
constants.add_argument(
    "-i", "--inertia", type=float, default=9.145e-6, help="Inertia of golf ball"
)

constants.add_argument(
    "--clubmass", type=float, default=0.2, help="Mass of club head (kg)"
)
...
constants.add_argument(
    ...
    type=float,
    default=9.81,
    help="For when we get a Mars base (m/s/s)",
)
constants.add_argument(
    "-d", "--density", type=float, default=1.225, help="Density of air (kg m^-3)"
)
constants.add_argument(
    "--viscosity", type=float, default=1.46e-5, help="Kinematic viscosity of air"
)

# Initial parameters
initialparams = parser.add_argument_group("Initial parameters")
initialparams.add_argument(
    "-yi", "--height", type=float, default=0, help="Initial height (m)"
)
...
loftangleparams.add_argument(
    "-li", "--loftinitial", type=float, default=10, help="Loft angle (initial)"
)
loftangleparams.add_argument(
    "-lf", "--loftfinal", type=float, default=35, help="Loft angle (final)"
)
loftangleparams.add_argument(
    "-st", "--step", type=float, default=5, help="Loft angle (step)"
)
...
# Input validation
assert (
    args.loftfinal > args.loftinitial
), "Final loft angle must be gretaer than initial loft angle!"
assert args.step != 0, "Step must be non-zero!"
assert (
    (args.loftfinal - args.loftinitial) / args.step
).is_integer(), "Step size must divide the change in loft angle!"
...
def ball_speed(theta):
    ...
    bfp = (
        args.vclub
        * np.sin(theta)
        / (1 + args.mass / args.clubmass + (args.mass * args.radius**2 / args.inertia))
    )
...
for theta in np.arange(args.loftinitial, args.loftfinal, args.step):
    ...
    ax.scatter(x, z, y, label="Loft angle: " + format(theta, ".1f"))
